...
export class EmprestimoService {
  ...
  baseUrl = "https://localhost:7203/consultasE";
  reservaUrl = "https://localhost:7203/reservas";
  ...
  getReservas(): Observable<Reserva[]>{
    return this.httpClient.get<Reserva[]>(this.reservaUrl).pipe( catchError(this.handleError)
    )
  }
  ...
  getConsultasE(): Observable<Emprestimo[]>{
    return this.httpClient.get<Emprestimo[]>(this.baseUrl).pipe( catchError(this.handleError)
    )
  }
  ...
  readById(codItem: string): Observable<Reserva> {
    console.log(codItem)
    const url = `${this.reservaUrl}/${codItem}`
  ...
  updateReserva(reserva: Reserva): Observable<Reserva> {
    const url = `${this.reservaUrl}/${reserva.codItem}`;
